...
class MultibancoConfig extends GatewayConfig
{
    ...
    {
        return ConfigVars::MULTIBANCO_CALLBACK_STRING . '&scp=' . $this->scope . '&scpcd=' . $this->scopeCode;
    }
    ...
    public function getOtherEntitiesSubEntityPairsInUse($thisEntity, $thisSubEntity)
    {
        ...
        $query = "SELECT scope, scope_id, value FROM {$tableName} WHERE path = :path";
        $binds = [
            ':path' => ConfigVars::DB_CONFIG_PREFIX_MULTIBANCO . ConfigVars::MULTIBANCO_ENTITY
        ];
        $entityRecords = $this->dbConn->fetchAll($query, $binds);
        ...
        $subEntityRecords = $this->dbConn->fetchAll($query, $binds);
