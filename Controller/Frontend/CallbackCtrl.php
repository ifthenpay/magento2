...
class CallbackCtrl extends Action
{
	...
	{
		if (!$storedPaymentData) {
			throw new \Exception('StoredPaymentData not found in local table.', ConfigVars::CB_FAIL_CODE_STORED_DATA_NOT_FOUND);
		}


		if ($storedPaymentData['status'] === ConfigVars::DB_STATUS_PAID) {
			throw new \Exception('Order already paid.', ConfigVars::CB_FAIL_CODE_ALREADY_PAID);
		}
		...
		if (!($requestData[ConfigVars::CB_PM] == 'mbway' ||
			...
			throw new \Exception('Invalid payment method.', ConfigVars::CB_FAIL_CODE_INVALID_PAYMENT_METHOD);
		}

		// is callback active?
		if (!$this->config->getIsCallbackActivated()) {
			throw new \Exception('Callback is not active.', ConfigVars::CB_FAIL_CODE_CALLBACK_NOT_ACTIVE);
		}
		...
		if ($requestData[ConfigVars::CB_ANTIPHISH_KEY] != $antiPhishingKey) {
			throw new \Exception('Invalid anti-phishing key.', ConfigVars::CB_FAIL_CODE_INVALID_ANTIPHISHING_KEY);
		}

		// is order id valid? does it exist?
		$this->order = $this->orderFactory->loadByIncrementId($storedPaymentData['order_id']);
		if (!$this->order->getId()) {
			throw new \Exception('Order not found.', ConfigVars::CB_FAIL_CODE_ORDER_NOT_FOUND);
		}
		...
		if ($requestAmount != $convertedOrderTotal) {
			throw new \Exception('Invalid amount.', ConfigVars::CB_FAIL_CODE_INVALID_AMOUNT);
		}
	}
}
